...
namespace TextEditor
{
    public partial class MainWindow : Window
    {
        ...
        private void ChangeIndent_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Відкриваємо діалогове вікно
                IndentDialog dialog = new IndentDialog();
                if (dialog.ShowDialog() == true)
                {
                    int indent = dialog.Indent;
                    TextBox textBox = TextBoxCheck();

                    if (textBox != null)
                    {
                        AddIndentToFirstLine(textBox, indent);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void AddIndentToFirstLine(TextBox textBox, int indent)
        {
            if (textBox != null && !string.IsNullOrEmpty(textBox.SelectedText))
            {
                ...
                textBox.Text = textBox.Text.Remove(startIndex, length).Insert(startIndex, modifiedText);
                textBox.SelectionStart = startIndex;
                textBox.SelectionLength = modifiedText.Length;
            }
        }
        private void UndoFormatting_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                TextBox textBox = TextBoxCheck();
                if (textBox != null)
                {
                    if (textBox.SelectionLength > 0)
                    {
                        ...
                        // Створити новий текст без форматування
                        textBox.Text = textBox.Text.Remove(startIndex, length).Insert(startIndex, selectedText);

                        // Відновити виділений текст
                        textBox.Select(startIndex, selectedText.Length);
                    }
                    else
                    {
                        MessageBox.Show("Будь ласка, виділіть текст, щоб скасувати форматування.", "Помилка", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
                }
            }
            catch (Exception ex)
